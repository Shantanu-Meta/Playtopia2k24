        ...
        <div className={`w-full lg:w-[90%] h-[70px] md:h-[100px] lg:h-[100px] md:px-[10px] lg:pt-0  lg:mx-[50px] flex justify-between items-center font-[Kollektif] fixed top-0 z-50`}>
            {windowWidth.windowWidth < 945 ? (
                ...
                <div className='flex justify-between items-center'>
                    {player.status ? (currentUser?.email === "shantanubhs1985@gmail.com" || currentUser?.email ==="arghajitsingharoy001@gmail.com" || currentUser?.email ==="raptic18@gmail.com" ||   currentUser?.email ==="projectpurpose634@gmail.com" ||   currentUser?.email ==="lbandopadhyay2@gmail.com" ? <Link to="/admin" className='profile border-2 border-white rounded-full p-2 text-white'><img src={currentUser?.photoURL ? currentUser.photoURL : "/user.jpg"} alt="User" className='rounded-full w-7 h-7 object-cover'/></Link> : <Link to="/user" className='profile border-2 border-white rounded-full p-1 text-white'><img src={currentUser?.photoURL ? currentUser.photoURL : "/user.jpg"} alt="User" className='rounded-full w-7 h-7 object-cover'/></Link>) : ''}
                    <Hamburger/>
                </div>
            </motion.header>
            ) : (
